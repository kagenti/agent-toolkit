...
extends and modifies it to work reliably within the Kagenti ADK while supporting context persistence, parallels run and Kagenti ADK primitives like LLM Proxy Service, File Storage or Environment Variables.

## Implementation Notes

- **Custom Backend implementation**
  - Introduces a custom `Backend` to store generated files in the Kagenti ADK file storage rather than
...
- **File handling updates**
  - Updates the `generate_social_image` and `generate_cover` tools to store assets using the
    `File.create(...)` API provided by the Kagenti ADK instead of writing to disk.

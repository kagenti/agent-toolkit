...
from adk_server.api.auth.errors import (
    IntrospectionDiscoveryError,
    IssuerDiscoveryError,
    JWKSDiscoveryError,
    UserInfoDiscoveryError,
)
from adk_server.configuration import Configuration, OidcProvider
from adk_server.domain.models.permissions import Permissions
...
def verify_internal_jwt(token: str, configuration: Configuration) -> ParsedToken:
    ...
    claims: JWTClaims = jwt.decode(
        ...
        claims_options={
            "sub": {"essential": True},
            "exp": {"essential": True},
            "iss": {"essential": True, "value": "adk-server"},
            "aud": {"essential": True, "value": "adk-server"},
        },
    )
    claims.validate()

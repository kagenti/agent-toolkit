...
from adk_server.configuration import (
    AuthConfiguration,
    Configuration,
...
async def rate_limited_server_client(
    adk_server, dependency_overrides_with_rate_limit, redis_config
) -> AsyncIterator[TestClient]:
    with adk_server(dependency_overrides=dependency_overrides_with_rate_limit) as client:
        try:
            yield client
        finally:
...
async def token_rate_limited_client(
    adk_server, token_rate_limit_config, redis_config, clean_up
) -> AsyncIterator[TestClient]:
    with adk_server(dependency_overrides=token_rate_limit_config) as client:
        try:

...
- [chat-ui example source](https://github.com/i-am-bee/agentstack/tree/main/apps/adk-ts/examples/chat-ui)
...
```bash
VITE_ADK_BASE_URL="http://localhost:8333"
VITE_ADK_PROVIDER_ID="your-provider-id"
```

- `VITE_ADK_BASE_URL`: URL of your actual Agent Stack server instance
...
- **Network errors when creating context/token**
  `VITE_ADK_BASE_URL` is wrong, unreachable, or points to a different environment. Verify the server URL and that the Agent Stack API is running.
...
- **Provider not found / invalid provider ID errors**
  `VITE_ADK_PROVIDER_ID` must match an existing provider on the target server. Confirm the ID in your Agent Stack instance.

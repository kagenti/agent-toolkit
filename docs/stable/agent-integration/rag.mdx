...
<Step title="Start Kagenti ADK with Docling enabled!!">
  Use `kagenti-adk platform start --set docling.enabled=true` to start Kagenti ADK with Docling.
</Step>
...
`EmbeddingServiceExtensionServer` and `EmbeddingServiceExtensionSpec` from `kagenti_adk.a2a.extensions`.
</Step>
...
Let's break down how each step can be implemented with the Kagenti ADK API.
...
Kagenti ADK uses [docling](https://docling-project.github.io/docling/) for extracting text out of documents in various
...
```python
from kagenti_adk.platform import File, Extraction
import asyncio
...
Now we need to generate embeddings for each chunk using the embedding service. Similarly to LLM, Kagenti ADK implements
...
```python
from openai import AsyncOpenAI
from kagenti_adk.platform import VectorStoreItem, File

...
```bash
uv python install --quiet --python-preference=only-managed --no-bin 3.14 && uv tool install --refresh --force --python-preference=only-managed --python=3.14 kagenti-adk && kagenti-adk self install
```

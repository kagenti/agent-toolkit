...
Once you've wrapped your agent with the Kagenti ADK server, you need to containerize it and deploy it, so that Kagenti ADK can run it as a managed service.

## Prerequisites
- Kagenti ADK installed ([Quickstart](/stable/introduction/quickstart))
- Agent wrapped with Kagenti ADK ([Wrap Existing Agents](/stable/deploy-agents/wrap-existing-agents) or [Build New Agent](/stable/deploy-agents/building-agents))

## Agent Deploy Process

To deploy your agent to Kagenti ADK, you need to create a github repository with your agent and then deploy from that repository to Kagenti ADK.

### 1. Create an Agent github repository

You can quickly create an agent repository by using the Kagenti ADK starter template. The [agentstack-starter template](https://github.com/i-am-bee/agentstack-starter) includes everything you need:
...
```bash
kagenti-adk add https://github.com/i-am-bee/agentstack-starter
```
...
The `kagenti-adk add` command supports various URL formats to specify the repository, version, and location of your agent's code.
...
kagenti-adk add agentstack.local/my-agent-abc123:latest
```

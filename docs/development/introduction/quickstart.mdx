...
```bash
uv python install --quiet --python-preference=only-managed --no-bin 3.13 && uv tool install --python-preference=only-managed --python=3.14 --refresh --prerelease allow --with kagenti-adk kagenti-cli --force && kagenti-adk platform start
```

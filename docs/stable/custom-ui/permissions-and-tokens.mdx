...
Kagenti ADK uses two types of authentication tokens to control access to platform resources:
...
Kagenti ADK's permission system has two independent dimensions that work together:
...
Kagenti ADK uses three role-based access levels:
...
<CodeGroup>
    ...
    from kagenti_adk.platform.context import Context, Permissions, ContextPermissions
    ...
    ```bash REST API
    # 1. Create a context
    curl -X POST "https://api.adk.example.com/api/v1/contexts" \
    ...
    curl -X POST "https://api.adk.example.com/api/v1/contexts/{context_id}/token" \
        -H "Authorization: Bearer YOUR_USER_TOKEN" \
        -H "Content-Type: application/json" \
        -d '{
...
When calling an agent using the Kagenti ADK, you pass the context token through extension configurations. Here's a complete example:
...
from kagenti_adk.platform import ModelProvider, Provider, ModelCapability
from kagenti_adk.platform.context import Permissions, ContextPermissions, Context
...
When you declare the `PlatformApiExtensionServer` in your agent function, the Kagenti ADK automatically authenticates your platform API calls using the context token that was passed through the extension metadata.
...
<Warning>
    ...
    with the Kagenti ADK.
</Warning>

The Kagenti ADK platform API accepts two types of tokens for authentication:

...
The platform API client provides typed access to Kagenti ADK server endpoints for managing contexts, files, providers, and other platform resources. This guide covers client initialization, authentication, response handling, and the complete endpoint reference.

The `buildApiClient` function creates a type safe HTTP client for interacting with the Kagenti ADK platform API. It returns an object that groups all platform endpoints into a single client. Every method returns an `ApiResult<T>` object so you can handle errors without throwing, or use `unwrapResult` to throw an `ApiErrorException` on failure.
...
const api = buildApiClient({
  baseUrl: "https://your-adk-instance.com",
...
- `baseUrl` (required): The base URL of your Kagenti ADK server instance.
...
```typescript
import { isHttpError, isValidationError } from "@kagenti/adk";

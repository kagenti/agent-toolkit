...
from kagenti_adk.a2a.extensions import ApprovalResponse
...
async def run(base_url: str = "http://127.0.0.1:10000"):
    async with httpx.AsyncClient(timeout=30) as httpx_client:
        ...
        approval_spec = kagenti_adk.a2a.extensions.ApprovalExtensionSpec.from_agent_card(card)

        if not approval_spec:
            raise ValueError(f"Agent at {base_url} does not support approval extension")

        approval_extension_client = kagenti_adk.a2a.extensions.ApprovalExtensionClient(approval_spec)

...
Connectors are remote MCP servers registered into the Kagenti ADK. Effectively, they take some responsibilities away from MCP Clients to improve user experience and reduce overhead within the Kagenti ADK.
...
```bash
npx -y @modelcontextprotocol/inspector --transport http --server-url http://adk-api.localtest.me:8080/api/v1/connectors/{id}/mcp
```
...
- **URL**: The MCP server endpoint (supports `http://` and `https://` schemes)
...
Connector presets are configurable via Helm values when deploying Kagenti ADK. The presets are defined in the `connector.presets` section of `values.yaml`:
...
The Kagenti ADK Python SDK provides a convenient wrapper around the connectors API, handling common patterns and reducing boilerplate. While it maps directly to API endpoints, the SDK adds extra functionality for smoother interaction patterns such as methods to wait until a connector reaches a desired state or until it is deleted.

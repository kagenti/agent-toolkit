...
| C3  | **SDK-first implementation.** Use `@kagenti/adk` SDK helpers (`handleAgentCard`, `resolveMetadata`, `resolveUserMetadata`, `handleTaskStatusUpdate`, `extractTextFromMessage`, `extractUiExtensionData`, `createAuthenticatedFetch`, `getAgentCardPath`, `streamResponseSchema`, `agentCardSchema`) instead of manually constructing protocol objects. Do not reimplement what the SDK provides. Note: the SDK does not provide an A2A transport client — the JSON-RPC + SSE streaming client must be implemented by the UI (see Step 4). |

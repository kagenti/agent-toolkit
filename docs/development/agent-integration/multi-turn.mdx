...
## Advanced Kagenti Framework Example

Here's a sophisticated example using the Kagenti Framework to build a multi-turn chat agent that leverages conversation history and LLM capabilities:
...
- **Framework Integration:** Leverages the Kagenti Framework for sophisticated agent capabilities
...
The Kagenti ADK provides a mechanism to delete history items from a specific point onward, allowing you to effectively “rewind” the conversation and replace a message with an edited version. 
...
async def edit_message_in_context(run_context: RunContext, id: UUID, new_text: str):

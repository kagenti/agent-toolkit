...
2. **Select and pin a trusted version (required).** If the current interpreter is unsupported, select a local interpreter that can install `kagenti-adk` and use that same interpreter for all dependency commands via `python -m pip`. If the project already pins `kagenti-adk` in its lockfile/constraints or active environment, use that compatible version and keep consistency with the project. If no version is present, use the latest compatible stable released `kagenti-adk` version from trusted PyPI metadata, then pin with `~=`.
   If the project requires direct `a2a-sdk` pinning, use a version compatible with the selected `kagenti-adk` dependency constraints.
...
- `kagenti-adk`
  ...
  - To resolve latest stable reliably, run `python -m pip install -qq --upgrade kagenti-adk`, read the installed version with `python -m pip show kagenti-adk`, then write `kagenti-adk~=<resolved_version>` to the existing manifest.
- `a2a-sdk`
  - If the project directly manages it: keep/add a version compatible with the selected `kagenti-adk`.
...
To figure out exact imports from installed libraries (`kagenti_adk`, `a2a`), you **must** use inline Python execution (`python -c`) to map imports without polluting the project repository. Do not guess import paths or rely solely on documentation if the package is installed.

Run this snippet **once** to crawl the installed package for `kagenti_adk` and print out all available classes and their exact import paths. Let the full output load into your context so you can resolve all multiple imports instantly without running the script again:

```bash
.venv/bin/python -c "import pkgutil, inspect, kagenti_adk as sdk; classes = {}; [classes.update({n: f'{m.name}.{n}' for n, o in inspect.getmembers(__import__(m.name, fromlist=['*']), inspect.isclass)}) for m in pkgutil.walk_packages(sdk.__path__, sdk.__name__ + '.')]; [print(path) for path in sorted(classes.values())]"
```

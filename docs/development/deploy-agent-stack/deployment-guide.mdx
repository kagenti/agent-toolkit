...
```shell
# kagenti-cli server login (if auth is enabled)
...
Keycloak is deployed externally (e.g. via kagenti-deps). The agentstack chart runs a provisioning job
to configure the agentstack realm, clients, and seed users.

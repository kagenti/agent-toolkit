...
To communicate with agents, you use the A2A protocol client to send messages and receive streaming events. The Kagenti ADK provides helpers that turn those events into UI updates. This guide shows how to integrate `@a2a-js/sdk` with the Kagenti ADK helpers, mirroring the same flow used in `agentstack-ui`.
